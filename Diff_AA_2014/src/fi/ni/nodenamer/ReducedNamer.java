...
public class ReducedNamer {
	...
	try {
	    ...
	    String local_uri = bn.getRDFClass_name() + "." + bn.getLiteralChksum() + "." + checksum.getChecksumValue();
	    bn.setLocal_uri(local_uri);	    
	    bn.setBased_on_nodecount(nodes.size()+end_checksumnodes);
	    bn.setActual_bits(actual_bits_reached);
	    bn.setUrlPathLength(actual_path_length);
	} catch (Exception e) {
	    e.printStackTrace();
	    System.exit(1);
	}
	...
	node_pathbits+=getBits(bn.getLiteral_prob())*0.4;  //!!! Tämä korjaa alkuperäisen differenssin!!
	//System.out.println("nodeb:"+node_pathbits);
	//node_pathbits += getBits(bn.getLiteral_prob());  // koska muuten tuplana!!
	...
	actual_bits_reached=(int)fixDistribution(node_pathbits);
        actual_path_length=path_length_limit;
